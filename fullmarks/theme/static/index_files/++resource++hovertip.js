...
jQuery.fn.hovertipActivate = function(config, targetSelect, tipPrepare, targetPrepare) {
  ...
  .each(function() {
      ...
      if (targets && targets.size()) {
        ...
        // is it bad form to call jq(this) here?
        if (!this.ht.tip)
          this.ht.tip = tipPrepare(jq(this), config);
      }
 ...
 * 
 * Invoke this function or one like it from your jq(document).ready(). 
 *  
...
function hovertipInit() {
   ...
   */
  window.setTimeout(function() {
    jq(hovertipSelect).hovertipActivate(hovertipConfig,
